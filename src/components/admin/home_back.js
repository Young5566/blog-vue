var CanvasParticle = (function(){
	...
	function canvasInit(canvasConfig){
		...
		var body = document.getElementById("right-page");
		...
		var canvas = {
			...
			config: {
				...
				stroke: canvasConfig.stroke || "121, 162, 185",
	...
	function canvasSize(canvas){
		...
		// 获取特定div的宽高
		var width = document.getElementById("right-page").style.width;
		var height = document.getElementById("right-page").style.height;
	...
	function drawLine(context, canvas, mouse){
		...
		for(var i = 0, len = canvas.config.count; i < len; i++){
			...
			for(var j = 0; j < len; j++){
				if(i != j){
          let dist = Math.round(canvas.points[i].x - canvas.points[j].x) * Math.round(canvas.points[i].x - canvas.points[j].x) +
			...
			// point to mouse
			if(mouse){
				let dist = Math.round(canvas.points[i].x - mouse.x) * Math.round(canvas.points[i].x - mouse.x) +
